...
let TravelManager = () => {
    ...
    const fetchData = () =>{
        ...
        .then((response) => {
          const data = response.data.data.filter(
            (travel) => travel.status_manager === 'pending'
          );
          setData(data);
        })
        .catch((error) => {
    ...
    const dataRows = data.map((x) => {
        return {
            ...
            status_manager: (
            <div>
                <CDBBtn color="success" variant="link" onClick={() => approve(x.travel_id)}>
                        Approve
                </CDBBtn>
                <CDBBtn color="danger" variant="link" onClick={() => reject(x.travel_id)}>
                    Reject
                </CDBBtn>
            </div>
            ),
